 ...
 * Date: 22/10/12
 */
public class TransferRequest extends AbstractSignedRequest {
	private static final String VERB = "transfer-request";

	private static final Logger log = Logger.getLogger("transfer-request");
	...
	public void requestTransfer(SessionPlace details, String fromUser, String toUser, int cents, Handler callback) {
		init(details);

		JSONObject message = Messages.createTransferRequest(fromUser, toUser, cents);
	...
	private static class Callback extends AbstractSignedRequest.Callback<Handler> {
		private Callback(TransferRequest req, Handler handler) {
	...
	static SafeUri url() {
		return UriUtils.fromSafeConstant(
				"/memphis/fridge/rest/account/transfer/json"
		);
	}
